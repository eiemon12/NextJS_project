...
export default function SignUp() {
    ...
    async function onSubmit(event: FormEvent) {
        ...
        if (Object.keys(validationErrors).length === 0) {
            ...
            try {
                const response = await axios.post('http://localhost:4000/customerSupport/createEMP', data);
                ...
                if (error.response && error.response.data && error.response.data.message) {
                    // Display the server validation messages
                    setErrors(error.response.data.message.reduce((acc: any, curr: string) => {
